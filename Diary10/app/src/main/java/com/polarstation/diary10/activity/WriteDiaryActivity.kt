...
class WriteDiaryActivity : BaseActivity(), View.OnClickListener {
                            ...
                            strInstance().reference.child(getString(R.string.fstr_diary_images)).child(uid()).child(diaryCreateTime).child(pageCreateTime).putFile(imageUri)
                                    .addOnSuccessListener {
                                        strInstance().reference.child(getString(R.string.fstr_diary_images)).child(uid()).child(diaryCreateTime).child(pageCreateTime).downloadUrl
                                                .addOnSuccessListener {
                                                    pushPage(applicationContext, dbInstance(), content, timeStamp, it.toString(), diaryKey(intent), title(intent), activityOp = Optional.of(this@WriteDiaryActivity))
                                                }
                                    }
                        else pushPage(applicationContext, dbInstance(), content, timeStamp, imageUri.toString(), diaryKey(intent), title(intent), activityOp = Optional.of(this@WriteDiaryActivity))
                    }
    ...
    companion object{
        @JvmStatic
        fun finishWithEditResult(activity: WriteDiaryActivity) {
